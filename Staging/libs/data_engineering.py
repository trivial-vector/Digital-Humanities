...
for y in years:
    ...
    for cy in census_years:
        ...
        if(cy == y):
            iscensus = 1
            # it should exit the for loop of census year (cy) - which will happen for yrs 1900,1910,1920
            break
    if(iscensus):
        # Yes it is census year then we have census race & name column to access
        df = df_basic_data_allyrs.loc[:, [
            "StNumClean", name, race, tenure, censusname, censusrace]]
    else:
...
for cy in census_years:
    ...
    for index, row in df_new.iterrows():
        dict_censuses_col[cy].update_many(
            {'Year': row[1], 'Street Address': row[-1], 'Census Name': row[0]}, {"$set": row.to_dict()}, upsert=True)
...
df_latlong["Street Address"] = df_latlong["Street Address"].str.replace(
    "Street", "St")
...
for y in years:
    for row in df_latlong.iterrows():
        data = dict(row[1])
        dict_censuses_col[y].update_many(
            {'Year': y, 'Street Address': row[1]["Street Address"]}, {"$set": data}, upsert=True)
